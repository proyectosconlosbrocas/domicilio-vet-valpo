...
- [x] T055 Corrección: el correo `domiciliovetvalpo@gmail.com` usado en T053 era falso/inexistente. Se da de baja esa cuenta (auth user + fila en `staff`) y se crea la cuenta real con **`mauro.carcamo89@gmail.com`** (`scripts/setup-real-staff.mjs`, con lógica de baja de cuentas viejas incluida). Verificado el flujo OTP + RLS directo contra la Auth API real (`verifyOtp` exitoso, fila propia de `staff` visible bajo RLS con la sesión resultante).

**Pendiente de acción manual del usuario**: confirmar que el correo real que llega a `mauro.carcamo89@gmail.com` muestra el código numérico (no solo un link para clickear) — el template por defecto de Supabase para "Magic Link" a veces no incluye `{{ .Token }}`. Si el correo real no trae el código, hay que agregarlo manualmente en el dashboard: **Authentication → Email Templates → Magic Link**, agregando `{{ .Token }}` al cuerpo. Además, hoy el envío de códigos está sujeto al límite por defecto de Supabase (pocos correos/hora) — se agotó durante las pruebas de esta sesión; para uso real sin cortes hay que configurar SMTP propio (Resend/Postmark), como ya estaba recomendado desde la Fase 10.
